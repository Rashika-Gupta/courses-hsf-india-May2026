...
G4bool SensitiveDetector::ProcessHits(G4Step* step, G4TouchableHistory*)
{
    ...
    auto pdg = pd->GetPDGEncoding();
    static int optical_photon_count = 0;
    auto* volume = pre->GetTouchableHandle()->GetVolume();
    auto* postVol = post->GetPhysicalVolume();
    std::string vol_name = volume->GetLogicalVolume()->GetName();
            ...
            if (!is_equal(track->GetVertexPosition(), pre->GetPosition()))
            {
                ...
                // Ideally we would do track->GetCurrentStepNumber() > 0, but
                // this information is not available in Celeritas
                auto* post = step->GetPostStepPoint();
                CELER_ASSERT(post);
                auto const pre_dir = to_array(pre->GetMomentumDirection());
                auto const post_dir = to_array(post->GetMomentumDirection());
                //   SD_1D_FILL(costheta, celeritas::dot_product(pre_dir,
                //   post_dir))
            }
